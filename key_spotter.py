...
class Keyword_Spotter:
    def __init__(self, model_path, audio_length, audio_path):
    ...
    def syntax_helper(self):
        print("Syntax: python key_spotter.py --al <audio_length> --model <model_path> --a <audio_path>")
    ...
    def display_result(self,infer):
        ...
        time1 = 0
        time2 = 2
        for disp in zip(self.result1,self.result2):
            if disp != None:
...
def main():
    ...
    ks = Keyword_Spotter(args.model, args.al, args.a)
    ks.file_checker()
    ks.audio_chunker()
    ptime = time.time()
    ks.process_asp()
